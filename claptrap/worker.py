...
from claptrap.compat import as_text, string_types, text_type
...
class Worker(object):
    redis_worker_namespace_prefix = 'claptrap:worker:'
    redis_workers_keys = 'claptrap:workers'
    ...
    def procline(self, message):
        ...
        """
        setprocname('claptrap: {0}'.format(message))
    ...
    def check_for_suspension(self, burst):
        """Check to see if workers have been suspended by `claptrap suspend`"""
        ...
        while not self._stop_requested and is_suspended(self.connection):
            ...
            if not notified:
                self.log.info('Worker suspended, run `claptrap resume` to resume')
...
class HerokuWorker(Worker):
    """
    Modified version of claptrap worker which:
