...
from claptrap import Connection, get_failed_queue, Queue
from claptrap.contrib.legacy import cleanup_ghosts
from claptrap.exceptions import InvalidJobOperationError
from claptrap.utils import import_attribute
from claptrap.suspension import (suspend as connection_suspend,
...
@click.option('--worker-class', '-w', default='claptrap.Worker', help='RQ Worker class to use')
@click.option('--job-class', '-j', default='claptrap.job.Job', help='RQ Job class to use')
@click.option('--queue-class', default='claptrap.Queue', help='RQ Queue class to use')
...
def worker(url, config, burst, name, worker_class, job_class, queue_class, connection_class, path, results_ttl,
    ...
    if is_suspended(conn):
        click.secho('RQ is currently suspended, to resume job execution run "claptrap resume"', fg='red')
        sys.exit(1)
    ...
    try:
        ...
        if sentry_dsn:
            ...
            from claptrap.contrib.sentry import register_sentry
...
def suspend(url, config, duration):
    """Suspends all workers, to resume run `claptrap resume`"""
...
def resume(url, config):
    """Resumes processing of queues, that where suspended with `claptrap suspend`"""
    connection = connect(url, config)
