...
from claptrap.compat import is_python_version
from claptrap.job import Job
from claptrap.cli import main
from claptrap.cli.helpers import read_config_file
...
class TestRQCli(RQTestCase):
    ...
    def test_worker(self):
        """claptrap worker -u <url> -b"""
        runner = CliRunner()
        result = runner.invoke(main, ['worker', '-u', self.redis_url, '-b'])
        self.assert_normal_execution(result)

    def test_exception_handlers(self):
        """claptrap worker -u <url> -b --exception-handler <handler>"""
        q = Queue()
    ...
    def test_suspend_and_resume(self):
        """claptrap suspend -u <url>
           claptrap resume -u <url>
        """
        runner = CliRunner()
    ...
    def test_suspend_with_ttl(self):
        """claptrap suspend -u <url> --duration=2
        """
        runner = CliRunner()
    ...
    def test_suspend_with_invalid_ttl(self):
        """claptrap suspend -u <url> --duration=0
        """
        runner = CliRunner()
